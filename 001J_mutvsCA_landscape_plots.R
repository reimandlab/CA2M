...
Roadmap_DNase = fread(pff("/data/001A_Roadmap_DNaseSeq_1MBwindow_processed.csv"))[,.SD,.SDcols=-c(1,2)]
...
make_plot = function(CA, mut, CA_label, mut_label){
	plot_dt = cbind.data.frame(chr = mut$chr, 
							   start = mut$start, 
							   CA = -1*scale(CA), 
							   mut = scale(mut[[3]]))
	plot_dt.m = reshape2::melt(plot_dt, 
					 id.vars = c("chr", "start"))
	...
	p = ggplot(plot_dt.m[plot_dt.m$chr == "chr2",], 
			   aes(x = start/(1e6), y = value, color = variable))+
	...
	scale_color_manual(labels = c(paste(CA_label,"chromatin accessibility\n(inverse scale)") , 
								  paste(mut_label,"mutation density")), 
					   values = c("Cyan", "Red"))+
...
mel_sig = fread(pff("/data/Sig_RMVs/Skin-Melanoma/SBS7a.csv"))
...
pdf(pff("/data/001J_landscape_plots/melanocyteCA_vs_melanoma7Amut.pdf"), width = 10, height = 4)
make_plot(melanocyte_CA, mel_sig, "Melanocyte", "Melanoma SBS7a")
dev.off()

####Breast AdenoCA vs BRCA
#Load in mutations file
breast_mut = fread(pff("/data/Sig_RMVs/Breast-AdenoCa/ALL.csv"))
...
pdf(pff("/data/001J_landscape_plots/BRCACA_vs_Breastmut.pdf"), width = 10, height = 4)
...
####GBM vs LGG
#Load in mutations file
GBM_mut = fread(pff("data/Sig_RMVs/CNS-GBM/ALL.csv"))
...
pdf(pff("/data/001J_landscape_plots/LGGCA_vs_GBMmut.pdf"),width=10,height=4)
make_plot(LGG_CA, GBM_mut, "LGG", "GBM")
dev.off()
						   
####### STAD vs STAD
STAD_mut = fread(pff("/data/Sig_RMVs/Stomach-AdenoCA/ALL.csv"))

#Compare with top predictor
top_pred = names(sort(sapply(colnames(Preds), function(x) cor(Preds[[x]], STAD_mut[[3]], 
															  method = "spearman"))))[1]
top_CA = Preds[[top_pred]]


pdf(pff("/data/001J_landscape_plots/STADCA_vs_STADmut.pdf"), width = 10, height = 4)
make_plot(top_CA, STAD_mut, "STAD", "Stomach-AdenoCA")
dev.off()
						   
####### Lymphoma
BNHL_mut = fread(pff("/data/Sig_RMVs/Lymph-BNHL/ALL.csv"))

#Compare with top predictor
top_pred = names(sort(sapply(colnames(Preds), function(x) cor(Preds[[x]], BNHL_mut[[3]], 
															  method = "spearman"))))[1]
top_CA = Preds[[top_pred]]


pdf(pff("/data/001J_landscape_plots/BcellCA_vs_BNHLmut.pdf"), width = 10, height = 4)
...
#Make CA line plot
pdf(pff("/data/001J_landscape_plots/STAD_CA_lineplot.pdf"), width = 10, height = 4)						   
...
#Make CA line plot
pdf(pff("/data/001J_landscape_plots/STAD_mut_barplot.pdf"), width = 10, height = 4)						   
