...
p1_rast = ggplot(q_val_dt.m, aes(x = cum_bp, y = -1*log10(value)))+
    ...
    scale_fill_manual(values = PCAWG_pal, 
					  breaks = levels(q_val_dt.m$colour_sig)[-5],
					  guide = F)+
...
pdf(pff("/data/005D_12cancertypes_manhattan_plot_sig_rast.pdf"), width = 9.5, height = 4)                             
p1_rast                                      
dev.off()   
...
pdf(pff("/data/005D_12cancertypes_manhattan_plot_LEGEND.pdf"), width = 4, height = 4)
grid.newpage()             
