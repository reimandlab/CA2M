...
pdf(pff("/data/003E_top_pred_facet_barplots.pdf"), width = 12, height = 6)
ggplot(plot_dt_select, aes(x = factor(order), 
	...
	geom_text(aes(y=value+SD+SD/12), colour = "black", size = 6)+
	geom_errorbar(aes(ymin = value - SD, ymax = value + SD), width=.2,
			 position=position_dodge(.9))+
	theme_bw()+
	theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1, size=  8, colour = "black"),
